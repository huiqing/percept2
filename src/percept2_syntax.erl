...
remove_function_clause(Node) ->
    FunName = percept2_syntax:function_name(Node),
    Cs = percept2_syntax:function_clauses(Node),
    Cs1=[case percept2_syntax:type(C) of
...
revert_clause_guard(E)->
    case is_tree(E) orelse is_wrapper(E) of
        true ->
            case  percept2_syntax:type(E) of
                disjunction -> percept2_syntax:revert_clause_disjunction(E);
                conjunction ->
                    %% Only the top level expression is
                    %% unfolded here; no recursion.
                    [percept2_syntax:conjunction_body(E)];
                _ ->
                    [[E]]       % a single expression
            end;
        false ->
