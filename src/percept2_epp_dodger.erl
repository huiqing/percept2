...
%%       File = file:filename()
%%       Options = [term()]
%%       Forms = [percept2_syntax:syntaxTree()]
%% 
...
%% `percept2_syntax:is_form/1'). Otherwise, `{error,
...
%%   percept2_syntax:text/1}), representing the raw token sequence of the
...
parse_form(Dev, L0, Parser, Options) ->
    ...
    Res = percept2_io:scan_erl_form(Dev, "", L0, TabWidth, FileFormat),
    case Res of    
        {ok, Ts, L1} ->
	    case catch {ok, Parser(Ts, Opt)} of
		    ...
		    {ok, percept2_syntax:set_pos(
			      percept2_syntax:text(tokens_to_string(Ts)),
			      start_pos(Ts, L1)),
		     L1};
                {parse_error, IoErr} ->
		    {error, {IoErr, {start_pos(Ts, L1), end_pos(Ts, L1)}}, L1};
...
parse_tokens(Ts, Fix) ->
      case percept2_parse:parse_form(Ts) of
        {ok, Form} ->
...
rewrite_form({function, _L, ?pp_form, _,
    ...
    percept2_syntax:set_pos(percept2_syntax:attribute(A, rewrite_list(As)), L0);
rewrite_form({function, _L, ?pp_form, _, [{clause, _, [], [], [A]}]} , L0) ->
    percept2_syntax:set_pos(percept2_syntax:attribute(A), L0);
...
%% Note: as soon as we start using percept2_syntax:subtrees/1 and similar
%% functions, we cannot assume that we know the exact representation of
%% the syntax tree anymore - we must use percept2_syntax functions to analyze
%% and decompose the data.
rewrite(Node) ->
    case percept2_syntax:type(Node) of
	atom ->
            case atom_to_list(percept2_syntax:atom_value(Node)) of
		?atom_prefix ++As ->
	...
	application ->
	    F = percept2_syntax:application_operator(Node),
	    case percept2_syntax:type(F) of
		atom ->
		    case percept2_syntax:atom_value(F) of
			?macro_call ->
                            [A| As] = percept2_syntax:application_arguments(Node),
                            M = percept2_syntax:macro(A, rewrite_list(As)),
                            percept2_syntax:copy_pos(Node, M);
			_ ->
			    rewrite_1(Node)
		    end;
		_ ->
...
make_macro_name(Name, ?atom_prefix) ->
    percept2_syntax:atom(Name);
make_macro_name(Name, ?var_prefix) ->
    percept2_syntax:variable(Name).

rewrite_macro(Node, As, Prefix) ->
    {L,_} = percept2_syntax:get_pos(Node),
    ...
    N = percept2_syntax:set_pos(make_macro_name(A,Prefix),{L,list_to_integer(Col) - 1}),
    percept2_syntax:set_pos(percept2_syntax:macro(N),{L,list_to_integer(Col) - 1}).

rewrite_1(Node) ->
    case percept2_syntax:subtrees(Node) of
	[] ->
	    Node;
	Gs ->
            Node1 = percept2_syntax:make_tree(percept2_syntax:type(Node),
					    [[rewrite(T) || T <- Ts]
					     || Ts <- Gs]),
	    percept2_syntax:copy_pos(Node, Node1)
    end.
...
fix_define([{atom, L, ?pp_form}, {'(', _}, {')', _}, {'->', _},
    ...
    A = percept2_syntax:set_pos(percept2_syntax:atom(define), La),
    Txt = percept2_syntax:set_pos(percept2_syntax:text(S), La),
    {form, percept2_syntax:set_pos(percept2_syntax:attribute(A, [N, Txt]), L)};
fix_define(_Ts) ->
    error.
...
unfold_function_names(Ns) ->
    F = fun ({{atom, Pos1, Atom},{integer, Pos2, Arity}}) ->
		N = percept2_syntax:arity_qualifier(percept2_syntax:set_pos(percept2_syntax:atom(Atom), Pos1),
                                                    percept2_syntax:set_pos(percept2_syntax:integer(Arity),Pos2)),
		percept2_syntax:set_pos(N, Pos1);
            ({{var, Pos1, MetaFunName}, {var, Pos2, MetaArity}}) ->
                N = percept2_syntax:arity_qualifier(percept2_syntax:set_pos(percept2_syntax:variable(MetaFunName), Pos1),
                                                    percept2_syntax:set_pos(percept2_syntax:variable(MetaArity),Pos2)),
		percept2_syntax:set_pos(N, Pos1)
        end,
    [F(N) || N <- Ns].
...
fix_pos([{'-', _}, {atom, _, import}|Ts], {attribute, Pos, Name, Data}) ->
    case Data of 
        {Module, Imports} ->
            ...
            Is2 = percept2_syntax:set_pos(percept2_syntax:list(Is1), Pos2),
...
fix_pos([{'-', _}, {atom, _, export}|Ts], {attribute, Pos, Name, Data}) ->
    ...
    Data1=percept2_syntax:set_pos(percept2_syntax:list(Es1), Pos2),
