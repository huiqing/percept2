...
add_new_export_var(_Pid, VarName) ->
    {SuffixNum, UsedNames, NewExportVars} = get(counter),
    put(counter, {SuffixNum, UsedNames, [VarName|NewExportVars]}).
    %% Pid ! {add, VarName}.

%%-spec get_new_export_vars(atom() | pid() | port() | {atom(),atom()}) -> [string()].
get_new_export_vars(_Pid) ->
    {_,_, NewExportVars} = get(counter),
    lists:reverse(NewExportVars).
