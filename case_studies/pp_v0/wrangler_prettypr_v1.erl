...
print_ast_and_get_changes(FileFmt, AST, Options, TabWidth) ->
    ...
    SubListSize = round(length(Fs)/parallelism()), 
    PPRes = percept2_utils:pmap(fun(F) ->
                              ?MODULE:print_a_form_and_get_changes(F, FileFmt, Options, TabWidth)
                           end, Fs, SubListSize),
    {FmStrs, C} = lists:unzip(PPRes),
...
%%     case all_whites_or_comments(Toks1) andalso 
%%         all_whites_or_comments(Toks2) of 
%% 	true ->
%% 	    true;
%% 	false ->
...
same(Toks1, Toks2, TabWidth) ->
    same(Toks1, Toks2, TabWidth, true, true).

same([], [], _TabWidth, _LeadWhts1, _LeadWhts2) -> true;
same([], Toks2, _TabWidth,  _LeadWhts1,  _LeadWhts2) -> 
    all_whites_or_comments(Toks2);
same(Toks1, [], _TabWidth, _LeadWhts1,  _LeadWhts2) -> 
    all_whites_or_comments(Toks1);
same([{whitespace, L, '\t'}|Toks1], Toks2, TabWidth, LeadWhts1,  LeadWhts2) -> 
    same(lists:duplicate(TabWidth, {whitespace, L, ' '})++Toks1,
         Toks2, TabWidth, LeadWhts1, LeadWhts2);
same(Toks1,[{whitespace, L, '\t'}|Toks2], TabWidth,  LeadWhts1,  LeadWhts2) -> 
    same(Toks1, lists:duplicate(TabWidth, {whitespace, L, ' '})++Toks2,
         TabWidth,  LeadWhts1,  LeadWhts2);
same([T1|Toks1], [T2|Toks2], TabWidth, true, true) ->
    case remove_loc(T1) == remove_loc(T2) of 
        true ->
            case is_whitespace(T1) orelse is_comment(T1) of 
                true ->
                    same(Toks1, Toks2, TabWidth, true, true);
                false ->
                    same(Toks1, Toks2, TabWidth, false, false)
            end;
        false ->
            false
    end;
same([{whitespace, _L, _}|Toks1], Toks2, TabWidth, false, false) ->
    same(Toks1, Toks2, TabWidth, false, false);
same(Toks1, [{whitespace, _L, _}|Toks2], TabWidth, false, false) ->
    same(Toks1, Toks2, TabWidth, false, false);
same([T1|Toks1], [T2|Toks2], TabWidth, false, false) ->
    case remove_loc(T1) == remove_loc(T2) of 
        true ->
            same(Toks1, Toks2, TabWidth, false, false);
        _ -> false
